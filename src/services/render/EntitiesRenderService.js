...
export default class EntitiesRenderService {
  ...
  setup() {
    this.entities = this.entityConfigs.map((configEntity) => {
      ...
      try {
        const entity = new Entity(configEntity, this.hass, this.map, this.historyService, this.dateRangeManager, this.linkedEntityService, this.isDarkMode, this.updateInitialView.bind(this));
        entity.setup();
        return entity; 
      } catch (e){
